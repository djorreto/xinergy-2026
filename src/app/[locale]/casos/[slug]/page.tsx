...
export async function generateStaticParams() {
  const { cases } = getContent(routing.defaultLocale);
...
export default async function CasoPage({ params }: Props) {
  ...
  const c = cases.find((x) => x.slug === slug);
  if (!c) notFound();
  const t = await getTranslations("ui.cases");

  const sections = [
    { label: t("challenge"), body: c.challenge },
    { label: t("approach"), body: c.approach },
  ];
